...
const BarChartHorizontal = ({ data,isHorizontal, categories, xAxisName, yAxisName, setBarChartState }) => {
  ...
  const options = {
    ...
    xaxis: {
      categories: categories || ["M", "T", "W", "T", "F", "S", "S"],
      title: {
        text: xAxisName || "X-Axis",  // Dynamic X-axis label
        style: {
          fontWeight: 600,
        },
      },
    ...
    yaxis: {
      title: {
        text: yAxisName || "Y-Axis",  // Dynamic Y-axis label
        style: {
          fontWeight: 600,
        },
      },
  ...
  const handleFilterChange = (newFilter) => {
    ...
    setBarChartState({ series: filteredData[newFilter] });
  };
    ...
    <div className="rounded-sm border border-stroke bg-white p-7.5 shadow-default dark:border-strokedark dark:bg-boxdark xl:col-span-4">
      ...
      <div>
        <div id="chartTwo" className="-ml-5 -mb-9">
          <ReactApexChart
            options={options}
            series={data}  // Dynamic data passed as prop
            type="bar"
            height={350}
          />
        </div>
